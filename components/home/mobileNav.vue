      ...
      >
        <div class="px-4 py-2">
          ...
          >
            ...
            >
              <span>{{ item.name }}</span>
              <!-- Show chevron only if there are subItems -->
              <svg
                v-if="item.subItems.length"
                :class="{
                  'transform rotate-180': isDropdownOpen(index),
                }"
            ...
            <!-- Sub-Items (Mobile) -->
            <div v-if="isDropdownOpen(index)" class="text-[#28574e]">
              <div
                v-for="(subItem, subIndex) in item.subItems"
                :key="subIndex"
                class="py-2 px-4 hover:bg-gray-100 cursor-pointer text-[#28574e]"
              >
                <!-- Check if subItem is an object with subItems or just a string -->
                <span class="text-[#89ccc0]">
                  {{ typeof subItem === 'string' ? subItem : subItem.name }}
                </span>
                <!-- Nested dropdown for sub-sub items -->
  ...
  const menuItems = [
    ...
    },
    {
      ...
      subItems: [
        {
          name: 'Shampoo',
          subItems: ['Dry Shampoo', 'Anti-Dandruff', 'Herbal Shampoo']
        },
        {
          name: 'Conditioner',
          subItems: ['Leave-in Conditioner', 'Deep Conditioner']
        },
        {
          name: 'Hair Oil',
          subItems: ['Coconut Oil', 'Argan Oil', 'Castor Oil']
        }
      ]
    },
    {
