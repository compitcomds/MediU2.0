<template>
  <div class="bg-white p-8 rounded-lg text-center max-w-3xl mx-auto">
    ...
    <h2 class="text-xl font-semibold text-[#238878] mb-2">Booking ID: {{ bookingID }}</h2>
    <p class="text-xl font-semibold text-[#238878] mb-4">Your Appointment has been booked successfully!</p>
    <p class="text-gray-600 mb-6">We have sent your booking information to your email address.</p>
    ...
    <div class="grid grid-cols-2 gap-6 mb-8">
      <div>
        ...
        <p class="text-lg text-[#238878]">{{ service.name }}</p>
      </div>
      
      <div>
        <h4 class="text-sm font-semibold text-gray-500 mb-2">Patient Name</h4>
        <p class="text-lg text-[#238878]">{{ customerName }}</p>
      </div>
      ...
      <div>
        <h4 class="text-sm font-semibold text-gray-500 mb-2">Total Amount Payable</h4>
        <p class="text-lg text-[#238878]">₹{{ service.price }}</p>
      </div>

      <div>
        <h4 class="text-sm font-semibold text-gray-500 mb-2">Notes</h4>
        <p class="text-lg text-[#238878]">{{ note }}</p>
      </div>
    </div>

    <!-- Action Buttons -->
    <div class="text-center mt-4">
      <button @click="goToHome" class="bg-[#238878] text-white px-6 py-2 rounded hover:bg-[#5c998e]">
        Go to Home
      </button>
    </div>
  </div>
